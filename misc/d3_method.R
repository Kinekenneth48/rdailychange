...
# d3_cands and dx which is derived from raw observations with no consecutive 0's.

# This function will take the TRUE d3_cands, sum up three consecutive days for
# the D3 Method. If that sum is positive, it will add it to the result, and
# falsify the three days used. If that sum is not positive it will do nothing.
# Looping through it will return the result.
Rcpp::cppFunction('NumericVector calc_d3_method(LogicalVector d3_cands,
                                                NumericVector dx) {
  int n = 3;
  NumericVector result;
  for (int i = 0; i < d3_cands.size(); i++) {
    if (d3_cands[i] == true) {
      int sum = 0;
      for (int j = i; j < i + n; j++) {
        sum += dx[j];
